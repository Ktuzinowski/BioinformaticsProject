...
def create_reference(work_path, file, soft):
    ...
    with open(work_path + f"References/Exon_maps/{file.replace(soft+'_', '').replace('_cds','').replace('_exon','')}" , "r") as file_R1:
        for ligne in file_R1:
            if "Exon_" in ligne:
                ligne = ligne.strip().split("\t")
...
def count_number_good_start_stop(workpath, soft, add_nuc, seq_type):
    ...
    liste_validated = []
    with open(f"/Users/kevintuzinowski/Studies/Benchmark_study-master/Sequence_types/{seq_type}.seq", "r") as file_R0:
